"""
Import Kafu Section 15 (pressure switches, diode blocks, interface modules).
Upserts A24; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section15-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Travel Alarm Control Switch",
        "Main Pump Regulator Pressure Switch",
        "Auxiliary Line Pressure Transducer",
        "Inline Protection Diode Block",
        "Dynamic Controller Interface Unit",
        "Hydraulic Override Safety Switch",
        "High-Low Speed Selector Switch",
        "Modular Interface Terminal Block",
        "Inline Harness Diode Module",
        "Swing Brake Release Pressure Switch",
        "Travel Speed Selection Switch",
        "Main Control Loop Diode Block",
        "Hydraulic Safety Lock Microswitch",
        "Electronic Monitoring Interface Board",
        "Hydraulic Lock Lever Microswitch",
        "Pump Control Interface Node",
        "Main Wiring Harness Diode Core",
        "Proportional Valve Interface Module",
        "Pilot Pressure Limit Switch",
        "Central Relay Interface Terminal Block",
        "Main Cabin Power Diode Array",
        "CAN-Bus Communication Interface Module",
        "Pilot Line Low Pressure Switch",
        "Travel Alarm Actuation Switch",
        "Modular Harness Diode Connector",
        "Main Pump Interface Logic Card",
        "Main System Protection Diode Node",
        "Digital Console Interface Board",
        "Travel Speed Limit Switch",
        "Circuit Isolation Interface Block",
        "Central Logic Communication Module",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Connector Spec|Terminal Core|Array|Board|Module|Panel|Unit)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Diode" in d:
        return "Diode Block / Array"
    if "Interface" in d or "Terminal" in d or "CAN-Bus" in d or "Communication" in d or "Logic" in d or "Node" in d:
        return "Electronic Interface Module"
    if "Switch" in d or "Transducer" in d or "Microswitch" in d:
        return "Pressure / Control Switch"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A24") or code in section_codes:
                continue
    ...
    s15 = sum(1 for p in products if p.get("section") == "Section 15")
    payload = {
        "status": "sections_2_to_15_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section15": s15,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section15_raw={len(rows)} section15={s15} kept={kept} total={len(products)}")
    for code in ["A01-1", "A23-1", "A24-1", "A24-3A", "A24-8", "A24-11", "A24-10A", "A24-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
