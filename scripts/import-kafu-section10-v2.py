"""
Import Kafu Section 10 (solenoid valves, flameout, coils).
Upserts A19; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section10-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Flameout Solenoid Valve (24V)",
        "Flameout Solenoid Valve",
        "Flameout Solenoid Actuator",
        "Flameout Solenoid (12V)",
        "Fuel Shut-off Solenoid (24V)",
        "Fuel Shut-off Solenoid",
        "Engine Shut-off Actuator",
        "Main Pump Solenoid Valve Block",
        "Main Pump Solenoid Valve",
        "Main Pump Proportional Valve",
        "Main Hydraulic Solenoid Block",
        "Solenoid Valve Assembly (5-Spool)",
        "Solenoid Valve Assembly (6-Spool)",
        "Solenoid Valve Block Assembly",
        "Solenoid Valve Replacement Coil",
        "Solenoid Valve Core Unit",
        "Solenoid Valve Block",
        "EPC Proportional Solenoid Valve",
        "Proportional Solenoid Valve",
        "Hydraulic Safety Lock Solenoid",
        "Hydraulic Lock Solenoid Valve",
        "Shift Solenoid Valve Assembly",
        "Electromagnetic Solenoid Coil",
        "Universal Circuit Block Diode",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    if not raw:
        return []
    raw = re.sub(
        r"\s+(Variant|Core|Series Core|Unit|Core Node|Stripe Node)$",
        "",
        raw,
        flags=re.I,
    )
    ...
    for p in parts:
        ...
        if re.fullmatch(r"\d+\s*[AVΩ]", t, re.I) or re.fullmatch(r"12V|24V", t, re.I):
            continue
        if len(t) < 2:
            continue
...
def normalize_category(desc: str) -> str:
    ...
    if "Flameout" in d or "Shut-off" in d or "Shut-off Actuator" in d:
        return "Flameout / Shut-off Solenoid"
    if "Proportional" in d or "EPC" in d:
        return "Proportional Solenoid Valve"
    if "Pump" in d or "Hydraulic" in d or "Shift" in d or "Spool" in d or "Valve" in d:
        return "Hydraulic Solenoid Valve"
    return d
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|KDRDE|RE)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A19") or code in section_codes:
                continue
    ...
    s10 = sum(1 for p in products if p.get("section") == "Section 10")
    payload = {
        "status": "sections_2_to_10_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section10": s10,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section10_raw={len(rows)} section10={s10} kept={kept} total={len(products)}")
    for code in ["A01-1", "A19-1", "A19-4A", "A19-10", "A19-14", "A19-21B", "A19-30"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
