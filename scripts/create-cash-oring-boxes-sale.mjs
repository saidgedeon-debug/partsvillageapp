/**
 * Cash client — O-ring boxes (Cat / Hitachi / Komatsu), paid & delivered.
 *
 * $15 × 3 = $45 total.
 *
 * Usage:
 *   node --env-file=.env.local scripts/create-cash-oring-boxes-sale.mjs --yes
 */
...
const ORDER_MARKER = "cash-oring-boxes-cat-hitachi-komatsu-v1";
const SALE_DATE = "2026-09-25";
const LINES = [
  { partNumber: "ORING-BOX-CATERPILLAR", name: "O-ring box Caterpillar", category: "O-Rings", qty: 1, unitPrice: 15 },
  { partNumber: "ORING-BOX-HITACHI", name: "O-ring box Hitachi", category: "O-Rings", qty: 1, unitPrice: 15 },
  { partNumber: "ORING-BOX-KOMATSU", name: "O-ring box Komatsu", category: "O-Rings", qty: 1, unitPrice: 15 },
];
...
async function main() {
  const parties = (await loadState("parties")) || { clients: [], suppliers: [] };
  const inventory = (await loadState("inventory")) || { overrides: {}, customParts: [] };
  ...
  if (
    ...
    console.log("Already exists for marker", ORDER_MARKER);
    process.exit(0);
  }

  const clients = parties.clients || [];
  let client =
    clients.find((c) => norm(c.name) === "cash") ||
    clients.find((c) => norm(c.name).includes("cash"));
  if (!client) {
    client = { id: mintId("cli"), name: "cash", email: "", phone: "", address: "", contactName: "" };
    clients.push(client);
  ...
  const createdAt = new Date(`${SALE_DATE}T10:35:00.000Z`);
  ...
  documents.push({
    ...
    customerNote: `Paid cash · delivered · ${ORDER_MARKER}`,
  });
  documents.push({
    id: receiptId,
    kind: "receipt",
  ...
  console.log("Invoice", invoiceId, "total", total, "Delivered");
}
