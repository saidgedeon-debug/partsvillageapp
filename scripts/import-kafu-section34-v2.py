"""
Import Kafu Section 34 (hydraulic filters, cabin air, fuel-water separators).
Upserts A43; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section34-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "High-Pressure Hydraulic Return Filter",
        "Primary Engine Air Filter Element",
        "Spin-On Fuel-Water Separator Canister",
        "Advanced High-Efficiency Oil Filter",
        "Cabin Recirculation Air Micro-Filter",
        "Pilot Line In-Line Hydraulic Filter",
        "Pleated Hydraulic Return Filter Cartridge",
        "Primary Spin-On Fuel Filter Element",
        "Charcoal Infused Cabin Air Element",
        "Hydraulic Tank Return Filter Element",
        "Dual-Stage Engine Air Cleaner Set",
        "Spin-On Fuel Pre-Filter & Separator",
        "Eco-Friendly Cartridge Oil Filter Unit",
        "Pilot Valve Circuit Hydraulic Element",
        "Glass-Fiber Hydraulic Return Filter",
        "Spin-On Secondary Fuel Filter Canister",
        "High-Capacity Outer/Inner Air Element Set",
        "Pleated Fiber Cabin Fresh Air Filter",
        "Hydraulic Oil Suction Strainer Block",
        "Fuel Filter & Separator Rebuild Core Set",
        "High-Pressure Hydraulic Pilot Line Element",
        "Combined Inner/Outer HVAC Filter Pack",
        "High-Density Hydraulic Return Element",
        "Fuel-Water Separator Bowl Spin-On Canister",
        "Primary Engine Air Induction Cleaner Set",
        "Hydraulic Tank Breather Filter Cap",
        "Integrated Hydraulic Return Filter Element",
        "Spin-On Engine Lube Oil Filter Canister",
        "Primary Air Induction Cleaner Filter Drum",
        "Hydraulic, Fuel, & Air Filter Complete Kit",
        "Fuel Water Separator Cartridge System",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Component Spec|Core Spec|Element Kit|Panel Array|Set|"
        r"Terminal Unit|Component Unit|Component|Core|Board|Module|Panel|Unit)$",
        "",
        raw,
        flags=re.I,
    )
    ...
    for p in parts:
        ...
        t = re.sub(r"\s+Set$", "", t, flags=re.I).strip()
        if len(t) < 2:
            continue
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Cabin" in d or "HVAC" in d or "Fresh Air" in d or "Micro-Filter" in d:
        return "Cabin Air Filter"
    if "Fuel" in d or "Water Separator" in d or "Separator" in d:
        return "Fuel / Water Separator"
    if "Air Filter" in d or "Air Cleaner" in d or "Air Element" in d or "Air Induction" in d:
        return "Engine Air Filter"
    if "Oil Filter" in d or "Lube Oil" in d:
        return "Engine Oil Filter"
    if "Hydraulic" in d or "Pilot" in d or "Breather" in d or "Strainer" in d:
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|YT|LC|LQ|KHR|AT|RE|ME|S\d|1R)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def parse_raw(text: str) -> list[dict]:
    ...
    for line in text.splitlines():
        ...
        if not line:
            continue
        if SECTION_RE.match(line) or line.startswith("This section") or line.startswith("This final"):
            continue
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A43") or code in section_codes:
                continue
    ...
    s34 = sum(1 for p in products if p.get("section") == "Section 34")
    payload = {
        "status": "sections_2_to_34_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section34": s34,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section34_raw={len(rows)} section34={s34} kept={kept} total={len(products)}")
    for code in ["A01-1", "A42-1", "A43-1", "A43-2", "A43-3A", "A43-14A", "A43-10A", "A43-23", "A43-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
