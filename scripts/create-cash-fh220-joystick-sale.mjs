/**
 * cash — Pusher joystick FH220 ×8 = $200, paid & picked up.
 * Usage: node --env-file=.env.local scripts/create-cash-fh220-joystick-sale.mjs --yes
 */
...
const ORDER_MARKER = "cash-pusher-joystick-fh220-sale-v1";
const CLIENT_NAME = "cash";
const SALE_DATE = "2026-09-26";
const LINES = [
  {
    partNumber: "PUSHER-JOYSTICK-FH220",
    name: "Pusher joystick FH220",
    category: "Misc",
    qty: 8,
    unitPrice: 25,
  },
...
async function main() {
  ...
  let client = clients.find((c) => norm(c.name) === "cash");
  if (!client) {
  ...
  const createdAt = new Date(`${SALE_DATE}T10:05:00.000Z`);
