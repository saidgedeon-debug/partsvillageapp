/**
 * Record a paid + picked-up sale for Nakhle Komatsu.
 *
 * Lines:
 *   Filter Komatsu mini valve  $5.00 × 2
 * Total $10.00 — Cash paid, fulfillment Picked up.
 *
 * Usage:
 *   node --env-file=.env.local scripts/create-nakhle-komatsu-sale.mjs --yes
 *   node --env-file=.env.local scripts/create-nakhle-komatsu-sale.mjs --yes --dry-run
 */
...
const ORDER_MARKER = "nakhle-komatsu-mini-valve-filter-sale-v1";
const CLIENT_NAME = "Nakhle Komatsu";
const SALE_DATE = "2026-09-25";

const LINES = [
  {
    partNumber: "FILTER-KOMATSU-MINI-VALVE",
    name: "Filter Komatsu mini valve",
    category: "Filters",
    qty: 2,
    unitPrice: 5,
  },
...
function findCustomPart(customParts, line) {
  ...
  return (
    ...
    customParts.find(
      (p) =>
        norm(p.name).includes("komatsu") &&
        norm(p.name).includes("mini") &&
        (norm(p.name).includes("valve") || norm(p.name).includes("filter")),
    )
  );
}
...
async function main() {
  ...
  let client =
    ...
    clients.find((c) => norm(c.name).includes("nakhle") || norm(c.name).includes("nakle"));
  if (!client) {
    client = {
      id: mintId("cli"),
      name: CLIENT_NAME,
  ...
  const createdAt = new Date(`${SALE_DATE}T10:20:00.000Z`);
