"""
Import Kafu Section 14 (relays, fuses, temperature controllers).
Upserts A23; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section14-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        ...
        "Main Cabinet Fuse Panel Module",
        "Cabin Climate Control Resistor",
        "Air Conditioning Controller Module",
        "High-Current Safety Circuit Fuse",
        "Heavy Duty Battery Relay",
        "Climate Control Step Resistor",
        "Main Cabinet Electronic Fuse Matrix",
        "Preheating System Glow Relay",
        "Battery Relay Solenoid Switch",
        "Central Safety Relay Control Module",
        "Electronic Air Conditioner Amplifier",
        "Glow Plug Preheater Controller",
        "Integrated System Protection Resistor",
        "Time Delay Safety Glow Relay",
        "Air Conditioner Step Motor Controller",
        "Electronic Fuse Distribution Unit",
        "Temperature Monitoring Circuit Board",
        "Air Conditioner Fan Control Resistor",
        "Complete Electronic Fuse Panel",
        "Main Interconnect Safety Switch Relay",
        "Digital Climate Temperature Amplifier",
        "Main Safety Start Cutoff Relay",
        "Electronic Ignition Safety Relay Module",
        "Automated Wiper Control Module Panel",
        "Integrated Thermal Cutout Relay",
        "Engine Safety Control Protection Block",
        "Digital HVAC Calibration Module",
        "High-Amperage Main Switch Relay",
        "Complete Heavy Vehicle Fuse Harness",
        "Central Safety Logic Circuit Relay",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Panel Core|Board|Module|Panel|Unit)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Fuse" in d or "Harness" in d:
        return "Fuse / Protection"
    if "Resistor" in d or "Climate" in d or "HVAC" in d or "Temperature" in d or "Air Conditioner" in d or "Air Conditioning" in d:
        return "Climate / Temperature Control"
    if "Relay" in d or "Glow" in d or "Preheater" in d:
        return "Relay / Glow Control"
    if "Controller" in d or "Amplifier" in d or "Module" in d or "Circuit" in d or "Board" in d:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A23") or code in section_codes:
                continue
    ...
    s14 = sum(1 for p in products if p.get("section") == "Section 14")
    payload = {
        "status": "sections_2_to_14_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section14": s14,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section14_raw={len(rows)} section14={s14} kept={kept} total={len(products)}")
    for code in ["A01-1", "A22-1", "A23-1", "A23-3A", "A23-8", "A23-11", "A23-10A", "A23-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
