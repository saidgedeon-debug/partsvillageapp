"""
Import Kafu Section 37 (hydraulic control valves, manifolds, pressure management).
Upserts A46; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section37-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Main Control Valve Primary Relief Valve",
        "Boom Regeneration Valve Assembly",
        "Line Anti-Cavitation Relief Valve (350 Bar)",
        "Pilot Pressure Reducing Valve Block",
        "Hydraulic Travel Speed Selection Valve",
        "Quick Coupler Auxiliary Control Block",
        "Main System Safety Relief Valve Cartridge",
        "Pilot Manifold Pressure Logic Block",
        "Inline Fluid Non-Return Check Valve",
        "Main Control Valve Port Relief Switch",
        "Travel Speed Selection Selector Block",
        "Main Line Pressure Safety Accumulator",
        "Pilot Line Logical EPC Reducing Valve",
        "Main Hydraulic Control Block Override",
        "Main Overload Port Relief Valve Cartridge",
        "Hydraulic Signal Convergence Manifold Block",
        "Pilot Circuit Pressure Control Valve",
        "Boom Hold-back Safety Valve Component",
        "Main Port Dual Stage Relief Switch",
        "Pilot Pressure Accumifold Terminal Block",
        "High-Pressure Main Line Safety Bypass Valve",
        "Proportional Pressure Control Logic Block",
        "Pilot Circuit Low Pressure Reducing Valve",
        "Straight Travel Logic Control Valve Block",
        "Main Port Overload Relief Valve assembly",
        "Auxiliary Hydraulic Spool Selector Block",
        "Hydraulic Overpressure Release Valve Node",
        "Main Valve Spool Pilot Interconnect Block",
        "High-Flow Two-Way Selector Control Valve",
        "Circuit Flow Regulator Valve Module",
        "Central Logic Proportional Relief Block",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Component Spec|Core Spec|Core Node|Terminal Core|Panel Unit|"
        r"Controller|Array|Board|Module|Panel|Unit|Component)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Check Valve" in d or "Non-Return" in d:
        return "Check / Non-Return Valve"
    if "Accumulator" in d:
        return "Pressure Accumulator"
    if "Manifold" in d or "Block" in d or "Logic" in d or "Selector" in d or "Override" in d:
        return "Control Manifold / Block"
    if "Relief" in d or "Reducing" in d or "Bypass" in d or "Overpressure" in d or "Anti-Cavitation" in d:
        return "Relief / Pressure Valve"
    if "Valve" in d:
        return "Hydraulic Control Valve"
    return d
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|YT|LC|LQ|KHR|AT|RE|ME|S\d)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def parse_raw(text: str) -> list[dict]:
    ...
    for line in text.splitlines():
        ...
        if (
            ...
            or line.startswith("This section")
            or line.startswith("This final")
        ):
            continue
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A46") or code in section_codes:
                continue
    ...
    s37 = sum(1 for p in products if p.get("section") == "Section 37")
    payload = {
        "status": "sections_2_to_37_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section37": s37,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section37_raw={len(rows)} section37={s37} kept={kept} total={len(products)}")
    for code in ["A01-1", "A44-1", "A46-1", "A46-3A", "A46-15", "A46-9", "A46-14A", "A46-10B", "A46-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
