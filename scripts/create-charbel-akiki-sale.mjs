/**
 * Record a paid + picked-up sale for Charbel Akiki.
 *
 * Lines:
 *   Komatsu joysticks pusher  $12.25 × 8  (= $98)
 *   Oil pressure               $8.00 × 1
 * Total $106.00 — Cash paid, fulfillment Picked up.
 *
 * Usage:
 *   node --env-file=.env.local scripts/create-charbel-akiki-sale.mjs --yes
 *   node --env-file=.env.local scripts/create-charbel-akiki-sale.mjs --yes --dry-run
 */
...
const ORDER_MARKER = "charbel-akiki-komatsu-joysticks-sale-v1";
const CLIENT_NAME = "Charbel Akiki";
const SALE_DATE = "2026-09-25";

const LINES = [
  {
    partNumber: "KOMATSU-JOYSTICK-PUSHER",
    name: "Komatsu joysticks pusher",
    category: "Misc",
    qty: 8,
    unitPrice: 12.25,
  },
  {
    partNumber: "OIL-PRESSURE",
    name: "Oil pressure",
    category: "Sensors",
    qty: 1,
    unitPrice: 8,
...
async function main() {
  ...
  let client =
    ...
    clients.find(
      (c) => norm(c.name).includes("charbel") && norm(c.name).includes("akiki"),
    );
  if (!client) {
    client = {
      id: mintId("cli"),
  ...
  const createdAt = new Date(`${SALE_DATE}T10:30:00.000Z`);
