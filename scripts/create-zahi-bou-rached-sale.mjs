/**
 * Zahi Bou Rached — ABC ×1 @ $87, paid & picked up.
 * Usage: node --env-file=.env.local scripts/create-zahi-bou-rached-sale.mjs --yes
 */
...
const ORDER_MARKER = "zahi-bou-rached-abc-sale-v1";
const CLIENT_NAME = "Zahi Bou Rached";
const SALE_DATE = "2026-09-25";
const LINES = [{ partNumber: "ABC", name: "ABC", category: "Misc", qty: 1, unitPrice: 87 }];
...
async function main() {
  ...
  let client =
    ...
    clients.find((c) => norm(c.name).includes("zahi") && norm(c.name).includes("rached"));
  if (!client) {
  ...
  const createdAt = new Date(`${SALE_DATE}T10:55:00.000Z`);
