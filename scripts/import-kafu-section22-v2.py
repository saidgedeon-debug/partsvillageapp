"""
Import Kafu Section 22 (pistons, liners, rings, internal power components).
Upserts A31; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section22-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Engine Piston & Pin Assembly",
        "Cylinder Liner (Finished Spec)",
        "Piston Ring Set (Three-Groove)",
        "Connecting Rod Bushing Kit",
        "Engine Crankshaft Thrust Washer",
        "Piston Wrist Pin Retaining Ring",
        "Engine Rebuild Cylinder Kit (Piston/Liner/Rings)",
        "High-Compression Engine Piston",
        "Cylinder Sleeve O-Ring Compression Seal",
        "Engine Piston Set (With Pin & Clips)",
        "Finished Wet Cylinder Liner",
        "Piston Ring Array (Single Cylinder Pack)",
        "Heavy Duty Compression Ring Matrix",
        "Connecting Rod Bearing Shell Set",
        "Complete In-Frame Engine Engine Rebuild Kit",
        "Piston & Ring Integration Assembly",
        "Dry-Type Cast Iron Cylinder Sleeve",
        "Crankshaft Main Journal Bearing Set",
        "Power Cylinder Piston Segment Array",
        "Chromed Hardened Piston Ring Set",
        "Cylinder Liner & Sleeve Interconnect",
        "Main Power Block Rebuild Component Array",
        "Aluminum Alloy Engine Piston Set",
        "Compression Piston Ring Grid",
        "Premium Cylinder Power Overhaul Kit",
        "Wet Liner Centrifugal Cast Unit",
        "Engine Internal Piston Rebuild Set",
        "Displacement Sleeve & Power Rig Component",
        "Compression Ring Repair Sub-Pack",
        "Heavy Duty Engine Piston & Sleeve Set",
        "Power Cylinder Internal Oil Scraping Ring",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Component Spec|Core Spec|Snap Pack|Terminal Unit|"
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Bearing" in d or "Bushing" in d or "Thrust Washer" in d:
        return "Bearing / Bushing"
    if "Ring" in d and ("Piston" in d or "Compression" in d or "Scraping" in d or "Retaining" in d):
        return "Piston Ring Set"
    if "Liner" in d or "Sleeve" in d:
        return "Cylinder Liner / Sleeve"
    if "Piston" in d or "Rebuild" in d or "Overhaul" in d or "Power" in d:
        return "Piston / Power Assembly"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A31") or code in section_codes:
                continue
    ...
    s22 = sum(1 for p in products if p.get("section") == "Section 22")
    payload = {
        "status": "sections_2_to_22_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section22": s22,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section22_raw={len(rows)} section22={s22} kept={kept} total={len(products)}")
    for code in ["A01-1", "A30-1", "A31-1", "A31-3A", "A31-9", "A31-13", "A31-14A", "A31-10C", "A31-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
