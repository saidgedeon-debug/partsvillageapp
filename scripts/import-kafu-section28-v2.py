"""
Import Kafu Section 28 (fuel injection pumps, injectors, high-pressure lines).
Upserts A37; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section28-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        ...
        "Mechanical Fuel Injector Assembly",
        "Common Rail Electronic Fuel Injector",
        "High-Pressure Common Rail Pump",
        "Fuel Primary Transfer Feed Pump",
        "High-Pressure Fuel Line Steel Tube Set",
        "Unit Injector / Common Rail Injector Set",
        "High-Pressure Fuel Injection Pump Unit",
        "Fuel Rail Pressure Limiter / Relief Valve",
        "Inline Mechanical Fuel Injection Pump",
        "Fuel Injector Nozzle Core Assembly",
        "High-Pressure Fuel Injection Pump (HP3)",
        "Priming Pump / Hand Fuel Feed Actuator",
        "Mechanical Fuel Injection Pump Assembly",
        "Common Rail Electronic Injector Valve",
        "High-Pressure Supply Fuel Pump Assembly",
        "High-Pressure Fuel Rail Injection Line Set",
        "High-Pressure Injection Pump Rebuild Unit",
        "Common Rail Fuel Injector System Array",
        "High-Pressure Common Rail Supply Pump",
        "Steel Fuel Feed Line & Tube Set Array",
        "High-Pressure Mechanical Fuel Pump",
        "Fuel Injection Nozzle Valve Array",
        "Denso Common Rail Fuel Injector Unit",
        "High-Pressure Common Rail Pump Unit",
        "Common Rail Fuel Injection Pump Unit",
        "High-Pressure Fuel Injector Array Rig",
        "Hand Primer Fuel Transfer Feed Pump",
        "Mechanical Fuel Injection Pump & Line Set",
        "Common Rail Electronically Controlled Injector",
    ],
    key=len,
    reverse=True,
)
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Line" in d or "Tube" in d or "Rail Injection Line" in d:
        return "High-Pressure Fuel Line"
    if "Feed" in d or "Priming" in d or "Primer" in d or "Transfer" in d:
        return "Fuel Feed / Primer Pump"
    if "Relief" in d or "Limiter" in d:
        return "Fuel Rail Relief Valve"
    if "Injector" in d or "Nozzle" in d:
        return "Fuel Injector"
    if "Pump" in d or "Injection" in d or "Common Rail" in d:
        return "Fuel Injection Pump"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A37") or code in section_codes:
                continue
    ...
    s28 = sum(1 for p in products if p.get("section") == "Section 28")
    payload = {
        "status": "sections_2_to_28_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section28": s28,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section28_raw={len(rows)} section28={s28} kept={kept} total={len(products)}")
    for code in ["A01-1", "A36-1", "A37-1", "A37-3A", "A37-9", "A37-10A", "A37-14A", "A37-23", "A37-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
            print(code, "|", p["description"], "|", p["oemNumbers"][:2], "|", p["compatibility"][:2], "|", p.get("pages"))
