"""
Import Kafu Section 32 (cylinders, piston rods, seal kits, bushings).
Upserts A41; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section32-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Boom Cylinder Rebuild Seal Kit",
        "Hardened Chrome Bucket Piston Rod",
        "High-Pressure Arm Cylinder Assembly",
        "Hardened Steel Bucket Link Bushing",
        "Arm Cylinder Wear Ring Array",
        "Dust Lip Wiper Seal Configuration Pack",
        "Arm Cylinder Total Rebuild Seal Kit",
        "Hardened Manganese H-Link Bushing Bearing",
        "Cushion Valve Piston Seal Configuration",
        "Complete Boom Hydraulic Cylinder",
        "Arm Cylinder Buffer Seal Ring Set",
        "Hardened Floating Pin Bushing Set",
        "Induction Hardened Arm Cylinder Rod",
        "Bucket Cylinder Complete Repair Seal Array",
        "Complete Cylinder Packing Gland Kit",
        "Bucket Linkage Steel Spacer Bushing Set",
        "High-Pressure Boom Cylinder Array",
        "Nitrile Rubber U-Cup Rod Seal Unit",
        "Boom Cylinder Polyurethane Seal Array",
        "Arm Attachment Link Bushing Repair Set",
        "Complete Bucket Hydraulic Cylinder Set",
        "Hardened Gland Nut End Cap Assembly Array",
        "Arm Cylinder Dynamic Packing Rebuild Set",
        "Induction Hardened Boom Cylinder Piston Rod",
        "Premium Hardened Steel Bucket Bushing Pack",
        "Dual-Lip Polyurethane Hydraulic Rod Seal",
        "Boom Attachment Arm Seal Array Kit",
        "Manganese Steel Linkage Bushing Rig",
        "Hardened Chrome Bucket Piston Rod Unit",
        "Heavy Duty Ram Cylinder & Bushing Line Set",
        "Cylinder End Gland Dynamic Buffer Seal",
    ],
    key=len,
    reverse=True,
)
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Bushing" in d or "Bearing" in d:
        return "Linkage Bushing / Bearing"
    if "Seal" in d or "Wiper" in d or "Wear Ring" in d or "Packing" in d or "Gland" in d:
        return "Cylinder Seal Kit"
    if "Piston Rod" in d or "Cylinder Rod" in d:
        return "Piston / Cylinder Rod"
    if "Cylinder" in d or "Ram" in d:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A41") or code in section_codes:
                continue
    ...
    s32 = sum(1 for p in products if p.get("section") == "Section 32")
    payload = {
        "status": "sections_2_to_32_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section32": s32,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section32_raw={len(rows)} section32={s32} kept={kept} total={len(products)}")
    for code in ["A01-1", "A40-1", "A41-1", "A41-3A", "A41-9", "A41-14A", "A41-10C", "A41-23", "A41-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
