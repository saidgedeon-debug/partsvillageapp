"""
Import Kafu Section 33 (track chains, rollers, idlers, sprockets).
Upserts A42; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section33-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Track Chain Assembly (45-Section Link)",
        "Bottom Track Roller (Dual-Flanged)",
        "Top Carrier Roller Assembly",
        "Front Tensioner Guide Idler Assembly",
        "Hardened Drive Sprocket Rim Segment",
        "Track Tension Recoil Spring Assembly",
        "Lubricated Track Link Chain Assembly",
        "Heavy Carrier Top Roller Array",
        "Heavy-Duty Drive Sprocket Segment",
        "Polyurethane Sealed Track Link Chain",
        "Heavy Bottom Track Roller Assembly",
        "Front Guide Idler Wheeled Drum Unit",
        "Segmented Tooth Drive Sprocket Rim",
        "Track Shoe Hex Fastener Bolt & Nut Pack",
        "Heavy Duty Track Link Assembly Set",
        "Dual Flange Lower Bottom Roller Unit",
        "Front Idler Yoke Tensioner Spring Block",
        "Hardened Drive Sprocket Core Wheel",
        "Grease-Sealed Track Chain Array Pack",
        "Double-Flanged Track Roller Core Block",
        "Complete Upper Structure Carrier Roller",
        "Cast Hardened Drive Sprocket Segment Array",
        "Heavy Duty Track Link Assembly Rail",
        "Top Carrier Roller Mount Bracket Setup",
        "Front Recoil Idler Wheel Cushion Unit",
        "Toothed Drive Sprocket Rim Segment Unit",
        "Crawler Underframe Track Chain Rail Assembly",
        "Lower Ground Track Roller Array Block",
        "Hardened Cast Segmented Drive Sprocket Pack",
        "Complete Heavy Track Link & Recoil Setup Set",
        "Top Track Alignment Roller Sub-Assembly",
    ],
    key=len,
    reverse=True,
)
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Sprocket" in d:
        return "Drive Sprocket"
    if "Idler" in d or "Recoil" in d or "Tension" in d:
        return "Idler / Tensioner"
    if "Carrier" in d or "Top" in d and "Roller" in d:
        return "Carrier Roller"
    if "Roller" in d:
        return "Track Roller"
    if "Chain" in d or "Track Link" in d or "Shoe" in d:
        return "Track Chain / Link"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A42") or code in section_codes:
                continue
    ...
    s33 = sum(1 for p in products if p.get("section") == "Section 33")
    payload = {
        "status": "sections_2_to_33_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section33": s33,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section33_raw={len(rows)} section33={s33} kept={kept} total={len(products)}")
    for code in ["A01-1", "A41-1", "A42-1", "A42-3A", "A42-9", "A42-14A", "A42-10C", "A42-23", "A42-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
