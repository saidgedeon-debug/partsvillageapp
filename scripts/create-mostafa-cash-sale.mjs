/**
 * Mostafa cash — ABC ×1 @ $60, paid & delivered.
 * Usage: node --env-file=.env.local scripts/create-mostafa-cash-sale.mjs --yes
 */
...
const ORDER_MARKER = "mostafa-cash-abc-sale-v1";
const CLIENT_NAME = "Mostafa cash";
const SALE_DATE = "2026-09-25";
const LINES = [{ partNumber: "ABC", name: "ABC", category: "Misc", qty: 1, unitPrice: 60 }];
...
async function main() {
  ...
  let client =
    ...
    clients.find((c) => (norm(c.name).includes("mostafa") || norm(c.name).includes("mustafa")) && norm(c.name).includes("cash"));
  if (!client) {
  ...
  const createdAt = new Date(`${SALE_DATE}T11:05:00.000Z`);
