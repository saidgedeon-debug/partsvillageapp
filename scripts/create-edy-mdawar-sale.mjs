/**
 * Edy Mdawar — PC300-5 boom Parker seal kit ×2 @ $125 = $250.
 * Paid & delivered.
 * Usage: node --env-file=.env.local scripts/create-edy-mdawar-sale.mjs --yes
 */
...
const ORDER_MARKER = "edy-mdawar-pc300-5-boom-parker-seal-kit-v1";
const CLIENT_NAME = "Edy Mdawar";
const SALE_DATE = "2026-09-26";
const LINES = [
  {
    partNumber: "PC300-5-BOOM-PARKER-SEAL-KIT",
    name: "PC300-5 boom Parker seal kit",
    category: "Seals",
    qty: 2,
    unitPrice: 125,
  },
...
async function main() {
  ...
  let client =
    ...
    clients.find(
      (c) =>
        (norm(c.name).includes("edy") || norm(c.name).includes("eddie")) &&
        (norm(c.name).includes("mdawar") || norm(c.name).includes("madawar")),
    );
  if (!client) {
