"""
Import Kafu Section 25 (camshafts, lifters, valve train timing).
Upserts A34; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section25-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Induction-Hardened Camshaft",
        "Mechanical Valve Lifter / Tappet",
        "High-Strength Engine Pushrod Set",
        "Camshaft Timing Helical Gear",
        "Camshaft Journal Bearing Shell Pack",
        "Rocker Arm Shaft Support Retainer",
        "Camshaft & Solid Lifter Overhaul Set",
        "Overhead Camshaft Assembly (4-Valve Head)",
        "Helical Timing Gear Train Sub-Assembly",
        "Billet Camshaft (Standard Journal Profile)",
        "Roller Valve Tappet / Lifter Set",
        "Solid Core Valve Pushrod Array",
        "Camshaft Drive Gear (Keyway Alignment)",
        "Camshaft Thrust Plate Alignment Washer",
        "Camshaft & Rocker Gear Complete Overhaul Kit",
        "Overhead Camshaft Unit (Single Profile)",
        "Valve Pushrod Assembly Set (16 Pieces)",
        "Valve Lifter Sintered Guide Shell Array",
        "Mid-Block Camshaft Segment Set",
        "Mechanical Valve Lifter Hardware Array",
        "High-Modulus Engine Valve Pushrod Set",
        "Gear Train Timing System Core Module",
        "Heat-Treated Alloy Steel Camshaft",
        "Solid Valve Tappet Mechanical Interface",
        "Overhead Valve Train Camshaft Array",
        "Precision Crank-to-Cam Idler Timing Gear",
        "Valve Timing Camshaft Hardware Block Set",
        "Valve Tappet & Cam Bushing Rebuild Rig",
        "Timing Idler Gear Hub Repair Sub-Pack",
        "Heavy Duty Overhead Camshaft & Lifter Set",
        "Overhead Valve Train Mechanical Pushrod Set",
    ],
    key=len,
    reverse=True,
)
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Gear" in d or "Timing" in d or "Idler" in d:
        return "Timing Gear / Drive"
    if "Lifter" in d or "Tappet" in d or "Pushrod" in d or "Rocker" in d:
        return "Lifter / Pushrod"
    if "Camshaft" in d or "Cam " in d:
        return "Camshaft"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A34") or code in section_codes:
                continue
    ...
    s25 = sum(1 for p in products if p.get("section") == "Section 25")
    payload = {
        "status": "sections_2_to_25_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section25": s25,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section25_raw={len(rows)} section25={s25} kept={kept} total={len(products)}")
    for code in ["A01-1", "A33-1", "A34-1", "A34-3A", "A34-9", "A34-14A", "A34-10C", "A34-23", "A34-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
