"""
Import Kafu Section 24 (crankshaft bearings, rod bearings, thrust washers).
Upserts A33; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section24-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Main Crankshaft Bearings Set (STD)",
        "Connecting Rod Bearings Shell Set",
        "Crankshaft Thrust Washers (Pair)",
        "Main Bearings Set (.25mm Oversize)",
        "Rod Bearings Shell Set (.25mm Oversize)",
        "Flanged Main Journal Bearing Unit",
        "Main & Rod Bearings Overhaul Kit",
        "Tri-Metal High-Load Rod Bearings",
        "Crankshaft Thrust Bearing Matrix",
        "Main Bearings Set (Bi-Metal Copper)",
        "Connecting Rod Bearing Pair",
        "Crankshaft Thrust Plate Washer Set",
        "Heavy Duty Main Journal Shell Kit",
        "Connecting Rod Small-End Bushing",
        "Bottom-End Engine Bearing Overhaul Kit",
        "Main Bearings Kit (Standard Set)",
        "Connecting Rod Bearings (Standard Set)",
        "Thrust Washer Alignment Block Set",
        "Crankshaft Main Bearing Segment Array",
        "Hardened Tri-Metal Rod Bearing Shell Set",
        "Crankshaft Axial Thrust Washer Pack",
        "Main & Rod Journal Friction Shell Array",
        "High-Alloy Main Crankshaft Bearing Pack",
        "Connecting Rod Bearing Matrix Set",
        "Premium Main & Rod Overhaul Journal Kit",
        "Semi-Circular Thrust Plate Washer Matrix",
        "Crankshaft Journal Bearing Component Set",
        "Main Bearing & Thrust Washer Integration Rig",
        "Connecting Rod Shell Bearing Repair Sub-Pack",
        "Heavy Duty Crankshaft & Rod Bearing Set",
        "Crankshaft Main Thrust Washer Repair Pack",
    ],
    key=len,
    reverse=True,
)
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Thrust" in d:
        return "Thrust Washer / Bearing"
    if "Rod" in d or "Connecting" in d or "Small-End" in d:
        return "Connecting Rod Bearing"
    if "Main" in d or "Crankshaft" in d or "Journal" in d or "Bearing" in d:
        return "Crankshaft Main Bearing"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A33") or code in section_codes:
                continue
    ...
    s24 = sum(1 for p in products if p.get("section") == "Section 24")
    payload = {
        "status": "sections_2_to_24_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section24": s24,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section24_raw={len(rows)} section24={s24} kept={kept} total={len(products)}")
    for code in ["A01-1", "A32-1", "A33-1", "A33-3A", "A33-9", "A33-14A", "A33-10C", "A33-23", "A33-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
