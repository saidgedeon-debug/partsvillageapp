/**
 * Mostafa to africa — A8VO200 plates, floating seal, coupling, ZF sensor.
 * Subtotal $850 − $50 discount = $800 paid & delivered.
 *
 * Usage: node --env-file=.env.local scripts/create-mostafa-to-africa-sale.mjs --yes
 */
...
const ORDER_MARKER = "mostafa-to-africa-a8vo200-sale-v1";
const CLIENT_NAME = "Mostafa to africa";
const SALE_DATE = "2026-09-25";
const DISCOUNT = 50;
const LINES = [
  { partNumber: "A8VO200-VALVE-PLATE-LEFT", name: "A8VO200 engrenax valve plate left", category: "Hydraulics", qty: 1, unitPrice: 200 },
  { partNumber: "A8VO200-VALVE-PLATE-RIGHT", name: "A8VO200 engrenax valve plate right", category: "Hydraulics", qty: 1, unitPrice: 200 },
  { partNumber: "FLOATING-SEAL-A", name: "Floating seal A", category: "Seals", qty: 1, unitPrice: 150 },
  { partNumber: "COUPLING-CAT-330C-GEAR", name: "Coupling CAT 330C + gear", category: "Couplings", qty: 1, unitPrice: 200 },
  { partNumber: "SENSOR-ZF", name: "Sensor ZF", category: "Sensors", qty: 1, unitPrice: 100 },
];
...
async function main() {
  ...
  let client =
    ...
    clients.find((c) => norm(c.name).includes("mostafa") && norm(c.name).includes("africa"));
  if (!client) {
  ...
  const subtotal = roundMoney(resolved.reduce((s, l) => s + l.qty * l.unitPrice, 0));
  const total = roundMoney(subtotal - DISCOUNT);
  const createdAt = new Date(`${SALE_DATE}T11:15:00.000Z`);
  ...
  documents.push({
    ...
    customerNote: `Paid $800 cash · $50 discount · delivered · ${ORDER_MARKER}`,
  });
  documents.push({
    id: receiptId,
    kind: "receipt",
  ...
  console.log("Invoice", invoiceId, "subtotal", subtotal, "discount", DISCOUNT, "total", total);
}
