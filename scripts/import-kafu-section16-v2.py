"""
Import Kafu Section 16 (electronic relays, timing modules, cabin controls).
Upserts A25; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section16-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Glow Plug Timer Delay Relay",
        "Intake Grid Preheater Element",
        "Windshield Wiper Delay Control Module",
        "Automated Throttle Safety Relay",
        "Smart Cab Lighting Control Card",
        "Heavy Duty Battery Isolation Switch",
        "Interior Fan Step Controller Relay",
        "Main Engine Safety Shutdown Module",
        "Flame Preheat Fuel Control Relay",
        "Battery Relay Solenoid Actuator",
        "Central Timing & Safety Relay",
        "Air Conditioner Fan Power Amplifier",
        "Intake Air Preheater Control Node",
        "Electronic Monitoring Logic Gateway",
        "Time Delay Flameout Relay Unit",
        "Wiper Step Frequency Controller",
        "Main Console Safety Relay Block",
        "Temperature Relay Override Circuit",
        "Air Conditioner High-Speed Relay",
        "Complete Main Cabin Relay Panel",
        "Main Interconnect Battery Disconnect Relay",
        "Digital Climate Control Interface Relay",
        "Main Pump Safety Interlock Relay",
        "Electronic Intermittent Wiper Relay",
        "Auto-Wiper Control Timing Unit",
        "Engine Idle-Speed Shutdown Relay",
        "Engine Safety Monitoring Relay Block",
        "Digital HVAC Calibration Circuit Board",
        "High-Amperage Solenoid Starter Switch",
        "Heavy Vehicle Flash & Blinker Relay",
        "Central Safety Electronic Timing Relay",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Control Node|Component|Core|Board|Module|Panel|Unit)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Wiper" in d or "Blinker" in d or "Lighting" in d or "Cabin" in d or "Climate" in d or "HVAC" in d or "Fan" in d:
        return "Cabin Control Interface"
    if "Preheater" in d or "Preheat" in d or "Glow" in d or "Flameout" in d:
        return "Preheater / Glow Control"
    if "Timing" in d or "Timer" in d or "Delay" in d:
        return "Timing Module / Relay"
    if "Relay" in d or "Switch" in d or "Shutdown" in d or "Interlock" in d:
        return "Electronic Relay"
    if "Module" in d or "Board" in d or "Gateway" in d or "Amplifier" in d or "Card" in d or "Circuit" in d:
        return "Cabin Control Interface"
    return d
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|LC|KHR|AT|RE)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A25") or code in section_codes:
                continue
    ...
    s16 = sum(1 for p in products if p.get("section") == "Section 16")
    payload = {
        "status": "sections_2_to_16_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section16": s16,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section16_raw={len(rows)} section16={s16} kept={kept} total={len(products)}")
    for code in ["A01-1", "A24-1", "A25-1", "A25-3A", "A25-8", "A25-11", "A25-10A", "A25-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
