"""
Import Kafu Section 35 (solenoid valves, diodes, switching relays).
Upserts A44; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section35-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Hydraulic Pump Proportional Solenoid",
        "Primary Fuel Shut-off Switch Actuator",
        "Safety Manifold Lock Solenoid Valve",
        "Main Pump Flow Regulator Solenoid",
        "Rotational Electronic Governor Core",
        "Miniature Cartridge Solenoid Stem",
        "Main Displacement Control Solenoid",
        "Pilot Block Manifold Cartridge Solenoid",
        "Automatic Fuel Cut-off Solenoid",
        "Engine Flameout Solenoid Switch (24V)",
        "Proportional EPC Solenoid Core Valve",
        "Multi-Station Solenoid Block (5-Spool)",
        "Multi-Station Solenoid Block (6-Spool)",
        "Variable Displacement Regulator Stem",
        "Fuel Supply Shut-off Solenoid Assembly",
        "Hydraulic Pump Dual Solenoid Valve Block",
        "Main System Relief Proportional Valve",
        "Pilot Line Selector Valve Interface Core",
        "Heavy Inline Engine Flameout Solenoid",
        "Pilot Line Fluid Distribution Solenoid Block",
        "Engine Fuel Manifold Shut-off Switch",
        "Proportional Pilot Pressure Reducing Valve",
        "Engine Governor Shut-off Solenoid Unit",
        "Pump Regulator Proportional Valve Shell",
        "High-Pressure Safety Selector Solenoid Core",
        "Pilot Manifold Cartridge Solenoid Valve",
        "Proportional Safety Lock Valve Assembly",
        "Hydraulic Main Manifold Regulator Coil",
        "Travel Track Direction Selector Valve",
        "Transmission Fluid Control Solenoid Unit",
        "Pilot Accumulator Line Cartridge Valve",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Component Spec|Core Spec|Spec Line|Block Node|Terminal Block|"
        r"Component Node|Component|Core|Board|Module|Panel|Unit)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Flameout" in d or "Fuel Shut" in d or "Cut-off" in d or "Shut-off" in d or "Governor" in d:
        return "Fuel Shut-off / Flameout Solenoid"
    if "Coil" in d:
        return "Solenoid Coil"
    if "Cartridge" in d or "Stem" in d or "Manifold" in d or "Pilot" in d:
        return "Pilot / Cartridge Solenoid"
    if "Solenoid" in d or "Valve" in d or "Actuator" in d or "Regulator" in d:
        return "Proportional Solenoid Valve"
    return d
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|YT|LC|LQ|KHR|AT|RE|ME|S\d|KDRDE)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A44") or code in section_codes:
                continue
    ...
    s35 = sum(1 for p in products if p.get("section") == "Section 35")
    payload = {
        "status": "sections_2_to_35_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section35": s35,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section35_raw={len(rows)} section35={s35} kept={kept} total={len(products)}")
    for code in ["A01-1", "A43-1", "A44-1", "A44-3A", "A44-9", "A44-15", "A44-10B", "A44-13", "A44-23"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
