"""
Import Kafu Section 11 (monitors, gauge clusters, displays).
Upserts A20; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section11-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "LCD Monitor Panel (New Style)",
        "LCD Monitor Display Panel",
        "LCD Monitor Cluster Panel",
        "LCD Dashboard Display",
        "LCD Monitor Unit",
        "Dashboard Display Cluster",
        "Dashboard Monitor Panel",
        "Dashboard Display Screen",
        "Dashboard Monitor Unit",
        "Gauge Cluster Monitor Panel",
        "Gauge Monitor Assembly",
        "Monitor Display Panel (Square Plug)",
        "Monitor Cluster Display Panel",
        "Monitor Gauge Dashboard",
        "High-Definition Monitor Unit",
        "High-Resolution Display Cluster",
        "Instrument Gauge Panel (I-ECU)",
        "Digital Hour Meter Cluster",
        "Digital Display Screen Panel",
        "Digital Display Screen Console",
        "Digital LCD Monitor Panel",
        "Digital Instrument Panel",
        "Display Panel Console Module",
        "Display Dashboard Cluster",
        "Smart Touch Monitor Console",
        "Smart LCD Monitor Console",
        "Intelligent Display Screen Module",
        "Instrument Cluster Unit",
        "Compact Cluster Unit",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    if not raw:
        return []
    raw = re.sub(r"\s+(Unit|Variant|Core)$", "", raw, flags=re.I)
...
def normalize_category(desc: str) -> str:
    d = desc
    if "LCD" in d or "Smart" in d or "Touch" in d or "High-Definition" in d or "High-Resolution" in d or "Intelligent" in d:
        return "LCD Monitor"
    if "Gauge" in d or "Cluster" in d or "Instrument" in d or "Hour Meter" in d:
        return "Gauge Cluster"
    if "Monitor" in d or "Display" in d or "Dashboard" in d or "Console" in d:
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|YT|LC|LQ|KHR|AT|RE)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A20") or code in section_codes:
                continue
    ...
    s11 = sum(1 for p in products if p.get("section") == "Section 11")
    payload = {
        "status": "sections_2_to_11_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section11": s11,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section11_raw={len(rows)} section11={s11} kept={kept} total={len(products)}")
    for code in ["A01-1", "A20-1", "A20-3A", "A20-7", "A20-11", "A20-15A", "A20-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
