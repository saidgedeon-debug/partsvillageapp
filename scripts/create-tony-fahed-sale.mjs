/**
 * Tony Fahed — ABC ×1 @ $30, paid & delivered.
 * Usage: node --env-file=.env.local scripts/create-tony-fahed-sale.mjs --yes
 */
...
const ORDER_MARKER = "tony-fahed-abc-sale-v1";
const CLIENT_NAME = "Tony Fahed";
const SALE_DATE = "2026-09-25";
const LINES = [{ partNumber: "ABC", name: "ABC", category: "Misc", qty: 1, unitPrice: 30 }];
...
async function main() {
  ...
  let client =
    ...
    clients.find((c) => norm(c.name).includes("tony") && (norm(c.name).includes("fahed") || norm(c.name).includes("fahd")));
  if (!client) {
  ...
  const createdAt = new Date(`${SALE_DATE}T11:00:00.000Z`);
