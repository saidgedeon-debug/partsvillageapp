"""
Import Kafu Section 18 (alternators, regulators, charging system nodes).
Upserts A27; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section18-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Charging Alternator (24V / 50A)",
        "Heavy-Duty Alternator (24V / 65A)",
        "Alternator Assembly (24V / 85A)",
        "Brushless Alternator Unit",
        "Solid-State Voltage Regulator",
        "Alternator Carbon Brush Kit",
        "Charging Alternator (24V / 80A)",
        "Heavy Infrastructure Alternator",
        "Electronic Voltage Regulator Module",
        "Charging Alternator (24V / 35A)",
        "Heavy-Duty Alternator Unit",
        "Integrated Fan Belt Alternator (60A)",
        "Alternator Voltage Regulator Block",
        "Charging Alternator Assembly",
        "Heavy Charging Alternator (50A)",
        "Alternator Assembly (24V / 60A)",
        "Charging System Voltage Regulator",
        "Charging Alternator Array (24V)",
        "Heavy Infrastructure Alternator (60A)",
        "Alternator Unit (With Internal Vacuum)",
        "Heavy Duty Alternator Unit",
        "Integrated Alternator Setup",
        "Charging System Alternator",
        "Charging Alternator System",
        "Integrated Alternator Rig",
        "Charging Alternator Module",
        "Heavy Mechanical Alternator (12V)",
        "Charging Node Internal Regulator",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Component Spec|Internal Spec|Component Unit|Terminal Unit|"
        r"Component|Core|Board|Module|Panel|Unit)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Brush" in d:
        return "Alternator Brush Kit"
    if "Regulator" in d:
        return "Voltage Regulator"
    if "Alternator" in d or "Charging" in d:
        return "Charging Alternator"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A27") or code in section_codes:
                continue
    ...
    s18 = sum(1 for p in products if p.get("section") == "Section 18")
    payload = {
        "status": "sections_2_to_18_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section18": s18,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section18_raw={len(rows)} section18={s18} kept={kept} total={len(products)}")
    for code in ["A01-1", "A26-1", "A27-1", "A27-3A", "A27-9", "A27-11", "A27-10A", "A27-19", "A27-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
