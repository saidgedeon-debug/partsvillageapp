/**
 * Khalil Hrajel — Hitachi O-ring box + oil level + delivery, paid & delivered.
 * Total $30 ($15 + $8 + $7).
 *
 * Usage: node --env-file=.env.local scripts/create-khalil-hrajel-sale.mjs --yes
 */
...
const ORDER_MARKER = "khalil-hrajel-hitachi-oring-oil-level-v1";
const CLIENT_NAME = "Khalil Hrajel";
const SALE_DATE = "2026-09-25";
const LINES = [
  { partNumber: "ORING-BOX-HITACHI", name: "O-ring box Hitachi", category: "O-Rings", qty: 1, unitPrice: 15 },
  { partNumber: "OIL-LEVEL-HYDRAULIC-HITACHI", name: "Oil level hydraulic Hitachi", category: "Sensors", qty: 1, unitPrice: 8 },
  { partNumber: "DELIVERY", name: "Delivery", category: "Misc", qty: 1, unitPrice: 7 },
];
...
async function main() {
  ...
  let client =
    ...
    clients.find((c) => norm(c.name).includes("khalil") && norm(c.name).includes("hrajel"));
  if (!client) {
  ...
  const createdAt = new Date(`${SALE_DATE}T10:50:00.000Z`);
