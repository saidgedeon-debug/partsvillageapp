/**
 * Abdo khalil — ABC ×1 @ $40, paid & picked up.
 * Usage: node --env-file=.env.local scripts/create-abdo-khalil-sale.mjs --yes
 */
...
const ORDER_MARKER = "abdo-khalil-abc-sale-v1";
const CLIENT_NAME = "Abdo khalil";
const SALE_DATE = "2026-09-25";
const LINES = [{ partNumber: "ABC", name: "ABC", category: "Misc", qty: 1, unitPrice: 40 }];
...
async function main() {
  ...
  let client =
    ...
    clients.find((c) => norm(c.name).includes("abdo") && norm(c.name).includes("khalil"));
  if (!client) {
  ...
  const createdAt = new Date(`${SALE_DATE}T11:10:00.000Z`);
