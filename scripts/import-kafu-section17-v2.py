"""
Import Kafu Section 17 (solenoid valves, flameout activators, electromagnetic coils).
Upserts A26; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section17-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        ...
        "Hydraulic Lock Manifold Valve",
        "Rotational Electronic Governor Valve",
        "Miniature Cartridge Solenoid Valve",
        "Main Pump Displacement Solenoid",
        "Pilot Manifold Cartridge Solenoid",
        "Automatic Fuel Stop Solenoid Actuator",
        "Fuel Shut-off Solenoid Switch (24V)",
        "Proportional EPC Solenoid Valve",
        "Solenoid Manifold Array (5-Valve Set)",
        "Solenoid Manifold Array (6-Valve Set)",
        "Variable Pilot Flow Valve Stem",
        "Fuel Supply Flameout Actuator",
        "Hydraulic Pump Dual Solenoid Block",
        "Main Relief Proportional Valve Valve",
        "Pilot Line Selector Solenoid Module",
        "Heavy Inline Flameout Solenoid",
        "Main Pilot Pump Solenoid Block",
        "Inline Engine Cutoff Solenoid Switch",
        "Proportional Pressure Valve Assembly",
        "Engine Shutdown Flameout Solenoid",
        "Proportional Pump Regulator Valve",
        "High-Pressure Safety Selector Solenoid",
        "Pilot Block Cartridge Solenoid",
        "Proportional Safety Lock Valve Block",
        "Hydraulic Manifold Regulator Coil",
        "Travel Direction Selection Solenoid",
        "Transmission Fluid Direction Solenoid",
        "Pilot Line Control Cartridge Stack",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Panel Unit|Terminal Unit|Component|Core|Board|Module|Panel|Unit)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Flameout" in d or "Fuel Shut" in d or "Fuel Stop" in d or "Cutoff" in d or "Shutdown" in d:
        return "Flameout / Fuel Shut-off"
    if "Coil" in d:
        return "Electromagnetic Coil"
    if "Manifold" in d or "Cartridge" in d or "Stem" in d or "Stack" in d:
        return "Solenoid Manifold / Cartridge"
    if "Solenoid" in d or "Valve" in d or "Actuator" in d or "Governor" in d:
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|LC|KHR|AT|RE|KDRDE)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A26") or code in section_codes:
                continue
    ...
    s17 = sum(1 for p in products if p.get("section") == "Section 17")
    payload = {
        "status": "sections_2_to_17_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section17": s17,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section17_raw={len(rows)} section17={s17} kept={kept} total={len(products)}")
    for code in ["A01-1", "A25-1", "A26-1", "A26-4B", "A26-8", "A26-11", "A26-10A", "A26-13", "A26-23"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
