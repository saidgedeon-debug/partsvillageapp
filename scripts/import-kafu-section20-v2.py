"""
Import Kafu Section 20 (engine starters, electric starters, solenoid assemblies).
Upserts A29; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section20-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Starting Motor Assembly (24V / 5.5kW)",
        "Heavy-Duty Electric Starter (24V)",
        "Starter Motor (10-Tooth Drive Pinion)",
        "Planetary Reduction Starter",
        "Engagement Solenoid Switch",
        "Starter Drive Pinion Gear Assembly",
        "Gear Reduction Starter Motor",
        "Heavy Infrastructure Starting Motor",
        "Magnetic Engagement Switch",
        "Reduction Starter Motor (24V / 4.5kW)",
        "Reduction Starter Motor (24V / 5.5kW)",
        "Heavy-Duty Electric Starter",
        "Integrated Overrunning Clutch Starter",
        "Internal Relay Solenoid Assembly",
        "Heavy Electric Starting Motor",
        "Direct-Drive Starter Assembly",
        "Charging Reduction Starting Motor",
        "Starter Mechanical Shift Solenoid",
        "Electric Starter Unit Array (24V)",
        "High-Torque Starter Motor",
        "Starting Motor Assembly",
        "Smart Reduction Starter Unit",
        "Electric Reduction Starter",
        "High-Torque Electric Starter",
        "Integrated Gear Starter Setup",
        "Reduction Gear Starting Motor",
        "Heavy Duty Starting System",
        "Integrated Starter Engine Rig",
        "Reduction Electric Starter Module",
        "Heavy Reduction Starter (12V)",
        "Solenoid Switch Shift Core",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Component Spec|Core Spec|Actuator|Component Kit|"
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Pinion" in d or "Clutch" in d:
        return "Starter Drive / Clutch"
    if "Solenoid" in d or "Engagement Switch" in d or "Magnetic" in d or "Shift Core" in d:
        return "Starter Solenoid Assembly"
    if "Starter" in d or "Starting" in d:
        return "Engine Starter Motor"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A29") or code in section_codes:
                continue
    ...
    s20 = sum(1 for p in products if p.get("section") == "Section 20")
    payload = {
        "status": "sections_2_to_20_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section20": s20,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section20_raw={len(rows)} section20={s20} kept={kept} total={len(products)}")
    for code in ["A01-1", "A28-24", "A29-1", "A29-3A", "A29-9", "A29-14A", "A29-10C", "A29-23", "A29-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
