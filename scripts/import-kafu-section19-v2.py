"""
Import Kafu Section 19 (AC compressors & climate control components).
Upserts A28; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section19-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "AC Compressor Assembly (24V)",
        "Heavy-Duty AC Compressor",
        "AC Compressor (7SBU16C Clutch)",
        "Balanced Air Compressor Unit",
        "Electromagnetic Clutch Assembly",
        "HVAC Blower Motor Resistor",
        "Climate Control AC Compressor",
        "Heavy Infrastructure AC Unit",
        "Thermal Expansion Valve",
        "AC Compressor (10S15C System)",
        "AC Compressor Assembly",
        "Eco-Drive AC Compressor Unit",
        "Pressure Cut-Off Safety Switch",
        "Heavy Scroll AC Compressor",
        "AC Compressor System (24V)",
        "Temperature Regulation Thermostat",
        "AC Compressor Array (24V)",
        "Smart Variable AC Compressor",
        "Heavy Duty Compressor Unit",
        "Integrated AC Compressor Setup",
        "AC Compressor System",
        "Integrated AC Rig Assembly",
        "AC Compressor Module",
        "Heavy Mechanical AC Unit (12V)",
        "HVAC Receiver Drier / Switch",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Component Spec|Core Spec|Compressor Hub|Component Unit|"
        r"Terminal Unit|Component|Core|Board|Module|Panel|Unit)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Clutch" in d:
        return "AC Clutch Assembly"
    if "Resistor" in d or "Thermostat" in d or "Expansion" in d or "Drier" in d or "Switch" in d:
        return "Climate Control Component"
    if "Compressor" in d or "AC " in d or "HVAC" in d:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A28") or code in section_codes:
                continue
    ...
    s19 = sum(1 for p in products if p.get("section") == "Section 19")
    payload = {
        "status": "sections_2_to_19_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section19": s19,
        "kept_prior": kept,
