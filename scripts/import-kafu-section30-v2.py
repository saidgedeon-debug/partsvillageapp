"""
Import Kafu Section 30 (hydraulic pumps, relief valves, regulators).
Upserts A39; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section30-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Main Hydraulic Axial Piston Pump",
        "Main Control Relief Valve (350 Bar)",
        "Hydraulic Pump Electronic Regulator",
        "Pilot Gear Pump Assembly",
        "Swashplate & Cradle Support Block",
        "Piston Shoe & Cylinder Block Rotary Kit",
        "Main Tandem Hydraulic Pump Unit",
        "Proportional Pump Regulator Control",
        "Pilot Line Main Relief Valve Stem",
        "Variable Displacement Hydraulic Pump",
        "Main Hydraulic Pump (Dual Stream)",
        "Main Control Valve Relief Cartridge",
        "Main Pump Assembly (HPV112 Series)",
        "Hydraulic Pump PC/LS Regulator Valve",
        "Main Hydraulic Pump Assembly",
        "Main Pump Regulator Assembly",
        "Main Control Overload Relief Valve",
        "Pilot Gear Pump (Triple Stage)",
        "Tandem Axial Piston Main Hydraulic Pump",
        "Main Control Block Relief Valve Cartridge",
        "Main Pump Variable Displacement Assembly",
        "Hydraulic Pump Electro-Proportional Regulator",
        "Twin Axial Piston Main Hydraulic Pump",
        "Main Control Block Relief Valve",
        "Variable Displacement Main Hydraulic Pump",
        "Hydraulic Pump Regulator Valve Unit",
        "Variable Axial Hydraulic Pump Unit",
        "Main Control Valve Primary Relief Stack",
        "Pilot Gear Charging Pump Assembly",
        "Main Piston Pump & Port Relief Valve Set",
        "Hydraulic Pump Flow Controller Regulator",
    ],
    key=len,
    reverse=True,
)
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Swashplate" in d or "Rotary Kit" in d or "Shoe" in d or "Cradle" in d:
        return "Pump Internal / Rotary Kit"
    if "Relief" in d or "Overload" in d:
        return "Main Relief Valve"
    if "Regulator" in d or "Controller" in d:
        return "Pump Regulator"
    if "Pilot" in d and "Pump" in d:
        return "Pilot Gear Pump"
    if "Pump" in d:
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|YT|LC|LQ|KHR|AT|RE|ME|S\d|HPV|K3V|PC/LS)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A39") or code in section_codes:
                continue
    ...
    s30 = sum(1 for p in products if p.get("section") == "Section 30")
    payload = {
        "status": "sections_2_to_30_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section30": s30,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section30_raw={len(rows)} section30={s30} kept={kept} total={len(products)}")
    for code in ["A01-1", "A38-1", "A39-1", "A39-3A", "A39-9", "A39-12", "A39-14A", "A39-10B", "A39-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
