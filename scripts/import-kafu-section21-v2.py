"""
Import Kafu Section 21 (engine gasket sets, seal kits, overhaul components).
Upserts A30; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section21-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Full Engine Overhaul Gasket Set",
        "Cylinder Head Gasket (Steel)",
        "Turbocharger Mounting Gasket Kit",
        "Fuel System Injector O-Ring Pack",
        "Valve Chamber Cover Gasket",
        "Crankshaft Front/Rear Oil Seal Kit",
        "Cylinder Head Gasket (Multi-Layer Steel)",
        "Exhaust Manifold Gasket Set",
        "Cylinder Head Gasket Composition Block",
        "Multi-Layer Steel Head Gasket Set",
        "Valve Stem Seal Kit Array",
        "Full Engine Gasket Ingestion Set",
        "Complete Motor Seals Rebuild Kit",
        "Cylinder Head Rebuild Gasket Pack",
        "Oil Pan Isolation Rubber Gasket",
        "Rebuild Gasket Component Array",
        "Block Overhaul Full Gasket Set",
        "Complete Engine Gasket Set",
        "Cylindrical Component Seal Assembly",
        "Overhaul Head Gasket Repair Pack",
        "Full Engine Gasket Rebuild Array",
        "Premium Cylinder Overhaul Seal Kit",
        "Mechanical Engine Gasket Array",
        "Engine Blocks Full Overhaul Set",
        "Total Block Integration Seal Rig",
        "Mechanical Gasket Sub-Assembly Pack",
        "Heavy Duty Head & Valve Gasket Set",
        "Cylinder Core Dynamic Fluid Seal",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Component Spec|Core Spec|Seals Pack|Terminal Unit|"
...
def normalize_category(desc: str) -> str:
    d = desc
    if "O-Ring" in d or "Oil Seal" in d or "Seal Kit" in d or "Seal Assembly" in d or "Fluid Seal" in d:
        return "Seal Kit"
    if "Overhaul" in d or "Rebuild" in d or "Full Engine" in d or "Gasket Set" in d:
        return "Engine Overhaul Gasket Set"
    if "Gasket" in d or "Seal" in d:
        return "Engine Gasket / Seal"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A30") or code in section_codes:
                continue
    ...
    s21 = sum(1 for p in products if p.get("section") == "Section 21")
    payload = {
        "status": "sections_2_to_21_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section21": s21,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section21_raw={len(rows)} section21={s21} kept={kept} total={len(products)}")
    for code in ["A01-1", "A29-1", "A30-1", "A30-3A", "A30-9", "A30-14A", "A30-10C", "A30-23", "A30-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
