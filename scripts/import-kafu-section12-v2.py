"""
Import Kafu Section 12 (wiring harnesses, plugs, control cables).
Upserts A21; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section12-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Heavy Sensor Connector Plug Kit",
        "Main Cab Wiring Harness",
        "Engine Electronic Harness",
        "Injector Component Harness",
        "Common Rail Wiring Assembly",
        "Common Rail Injection Harness",
        "Dynamic Fuel Injection Cable",
        "Main Engine Control Harness",
        "Governor Link Communication Cable",
        "Main Pump Control Wiring Loop",
        "Cabinet Dashboard Assembly Harness",
        "Main Controller Ingestion Harness",
        "High-Pressure Common Rail Wire Loom",
        "Engine Overhaul Terminal Wire Kit",
        "External Pump Control Wire Set",
        "Multi-Function Cabin Console Harness",
        "Interconnect Engine Control Loom",
        "Sensor Distribution Extension Lead",
        "Cabinet Interface Connection Cable",
        "Digital Common Rail Engine Harness",
        "Processing Unit Core Wire Loom",
        "Smart System Integration Harness",
        "Engine Frame Harness Array",
        "Electronic Governor Connector Cable",
        "Main Frame Electrical Control Loom",
        "Main System Distribution Wire Loom",
        "Engine Electrical Junction Loom",
        "Machine Integration Wire Harness",
        "Core Frame Electrical Array",
        "Alternator & Starter Heavy Cable Set",
        "Sensor Bus Interconnect Line",
    ],
    key=len,
    reverse=True,
)
...
BRANDS = (
    ...
    r"Deutz|JCB|Daewoo|XCMG|Sumitomo|Universal|Deutsch|CAT\b"
)
...
def parse_oems(raw: str) -> list[str]:
    if not raw:
        return []
    raw = re.sub(
        r"\s+(Variant|Core|Block Spec|Unit Spec|Line Spec|Interface Set|"
        r"Assortment)$",
        "",
        raw,
        flags=re.I,
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Plug Kit" in d or "Connector" in d or "Extension Lead" in d:
        return "Sensor Connector / Plug Kit"
    if "Common Rail" in d or "Injection" in d or "Injector" in d or "Fuel Injection" in d:
        return "Common Rail / Injection Harness"
    if "Cab" in d or "Cabin" in d or "Dashboard" in d or "Console" in d:
        return "Cabin Wiring Harness"
    if "Pump" in d or "Governor" in d or "Controller" in d or "Engine" in d or "Frame" in d:
        return "Engine / Control Harness"
    if "Harness" in d or "Loom" in d or "Wire" in d or "Cable" in d or "Array" in d:
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|YT|LC|LQ|KHR|AT|RE|Deutsch)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A21") or code in section_codes:
                continue
    ...
    s12 = sum(1 for p in products if p.get("section") == "Section 12")
    payload = {
        "status": "sections_2_to_12_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section12": s12,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section12_raw={len(rows)} section12={s12} kept={kept} total={len(products)}")
    for code in ["A01-1", "A21-1", "A21-3A", "A21-8", "A21-11", "A21-15", "A21-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
