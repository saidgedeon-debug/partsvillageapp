"""
Import Kafu Section 13 (solenoid coils, cartridges, pilot manifolds).
Upserts A22; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section13-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Proportional Solenoid Valve Coil",
        "Hydraulic Pump Control Coil",
        "Main Pump Solenoid Valve Stem",
        "Rotary Solenoid Coil Unit",
        "Cartridge Valve Replacement Block",
        "Heavy Proportional Stem Kit",
        "Main Pump Control Solenoid Coil",
        "Pilot Manifold Cartridge Valve",
        "Proportional Brake Solenoid Coil",
        "Main Pump EPC Solenoid Valve Coil",
        "Proportional Cartridge Solenoid",
        "Pilot Block Valve Solenoid Unit",
        "EPC Block Replacement Coil",
        "Proportional Flow Valve Stem",
        "Hydraulic Pump Stepping Coil Unit",
        "Main Pump Proportional Coil Block",
        "Cartridge Valve Solenoid Stem",
        "Multi-Station Solenoid Pilot Coil",
        "Pilot Control Solenoid Valve Coil",
        "Proportional Pressure Control Stem",
        "High-Pressure Safety Shut-Off Coil",
        "Proportional Reducing Valve Coil",
        "Main Pump Proportional Solenoid Unit",
        "Pilot Valve Block Solenoid Stem",
        "Proportional Flow Valve Assembly",
        "Pilot Cartridge Reducing Valve",
        "Pilot Line Distribution Valve Coil",
        "Main Pump Regulator Solenoid Coil",
        "Hydraulic Lock Cartridge & Coil",
        "Transmission Solenoid Cartridge Unit",
        "Main Hydraulic Manifold Control Coil",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    if not raw:
        return []
    raw = re.sub(
        r"\s+(Variant|Node|Block Spec|Unit Spec|Line Spec)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    ...
    if "Cartridge" in d or "Stem" in d or "Manifold" in d or "Pilot" in d:
        return "Cartridge / Pilot Valve"
    if "Coil" in d:
        return "Solenoid Coil"
    if "Valve" in d or "Solenoid" in d:
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|KDRDE|KHR|AT|RE)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A22") or code in section_codes:
                continue
    ...
    s13 = sum(1 for p in products if p.get("section") == "Section 13")
    payload = {
        "status": "sections_2_to_13_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section13": s13,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section13_raw={len(rows)} section13={s13} kept={kept} total={len(products)}")
    for code in ["A01-1", "A22-1", "A22-3A", "A22-8", "A22-11", "A22-10A", "A22-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
