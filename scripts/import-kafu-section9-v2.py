"""
Import Kafu Section 9 (throttle motors, actuators, cables).
Upserts A18; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section9-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Throttle Stepper Motor (Dual Cable)",
        "Throttle Stepper Motor",
        "Throttle Motor Assembly (7-Pin)",
        "Throttle Motor Assembly",
        "Throttle Motor Core Unit",
        "Throttle Motor Unit",
        "Throttle Actuator Motor",
        "Throttle Actuator (Small Core)",
        "Electronic Governor Actuator",
        "Electronic Throttle Actuator",
        "Governor Control Cable",
        "Accelerator Control Cable",
        "Universal Accelerator Cable",
        "Throttle Position Sensor",
        "Throttle Sensor Core",
...
def parse_oems(raw: str) -> list[str]:
    if not raw:
        return []
    raw = re.sub(
        r"\s+(Variant|Core|Series Core|Engine Core|Rail|Link Node|"
        r"Multi-Fit Ball Joint Head)$",
        "",
        raw,
        flags=re.I,
    )
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Cable" in d:
        return "Governor / Accelerator Cable"
    if "Position Sensor" in d or "Sensor Core" in d or "Potentiometer" in d:
        return "Throttle Position Sensor"
    if "Governor" in d or "Electronic Throttle" in d or "Actuator" in d:
        return "Throttle Actuator"
    if "Stepper" in d or "Throttle Motor" in d or "Stepper Motor" in d:
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|YT|LQ|KHR|RE)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A18") or code in section_codes:
                continue
    ...
    s9 = sum(1 for p in products if p.get("section") == "Section 9")
    payload = {
        "status": "sections_2_to_9_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section9": s9,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section9_raw={len(rows)} section9={s9} kept={kept} total={len(products)}")
    for code in ["A01-1", "A18-1", "A18-4A", "A18-8", "A18-12", "A18-27", "A18-32"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
