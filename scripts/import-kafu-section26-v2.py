"""
Import Kafu Section 26 (water pumps, oil pumps, cooling system parts).
Upserts A35; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section26-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        ...
        "Gear-Driven Engine Lubrication Oil Pump",
        "Viscous Fan Clutch Assembly",
        "Dual-Valve Thermostat Unit (82°C)",
        "Oil Pump Relief Valve Spring Pack",
        "Water Pump Impeller Repair Seal Kit",
        "Heavy Duty Water Pump Assembly",
        "High-Pressure Lubrication Gear Pump",
        "Cooling System Thermostat Housing",
        "Belt-Driven Cooling Water Pump",
        "Main Journal Lubrication Oil Pump",
        "Bi-Metal Fan Clutch Drive Unit",
        "High-Efficiency Flow Water Pump",
        "Thermostat Regulator Valve Core (76.5°C)",
        "Heavy Mechanical Water Pump Unit",
        "Rotary Trochoid Engine Oil Pump",
        "High-Flow Water Pump System",
        "Viscous Drive Radiator Fan Clutch",
        "Cooling Water Pump Setup Array",
        "Lubricating Internal Sump Oil Pump",
        "Heavy Water Pump Assembly",
        "Sump Lubrication Oil Pump Rig",
        "High-Delivery Coolant Water Pump",
        "Sump Lubrication Oil Pump",
        "Integrated Water Pump Unit",
        "High-Capacity Lubricant Oil Pump",
        "Engine Cooling Water Pump Block Set",
        "Lubricating Sump Engine Oil Pump Rig",
        "Viscous Fan Clutch Drive Component Unit",
        "Heavy Duty Water Pump & Thermostat Set",
        "Internal Engine Gear Lubrication Pump",
    ],
    key=len,
    reverse=True,
)
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Thermostat" in d:
        return "Thermostat / Cooling Control"
    if "Fan Clutch" in d:
        return "Fan Clutch"
    if "Oil Pump" in d or "Lubrication" in d or "Lubricant" in d or "Lubricating" in d:
        return "Engine Oil Pump"
    if "Water Pump" in d or "Coolant" in d or "Cooling" in d:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A35") or code in section_codes:
                continue
    ...
    s26 = sum(1 for p in products if p.get("section") == "Section 26")
    payload = {
        "status": "sections_2_to_26_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section26": s26,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section26_raw={len(rows)} section26={s26} kept={kept} total={len(products)}")
    for code in ["A01-1", "A34-1", "A35-1", "A35-3A", "A35-9", "A35-14A", "A35-10C", "A35-23", "A35-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
