"""
Import Kafu Section 27 (turbochargers, mufflers, exhaust system parts).
Upserts A36; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section27-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Turbocharger Unit (Wastegate-Regulated)",
        "Engine Silencer / Back-Pressure Muffler",
        "High-Temperature Exhaust Manifold Log",
        "Turbocharger Actuator Diaphragm",
        "Exhaust Pipe Clamp Connection Ring",
        "Multi-Layer Exhaust Insulation Gasket",
        "Heavy-Duty Turbocharger System",
        "Replacement Exhaust Silencer Muffler",
        "High-Temp Manifold Ingestion Gasket Set",
        "Oil-Cooled Turbocharger Core",
        "Heavy Carbon Steel Exhaust Muffler",
        "Exhaust Manifold Stud & Nut Fastener Set",
        "High-Pressure Wastegate Turbocharger",
        "Flexible Exhaust Expansion Joint Bellows",
        "Twin-Scroll Engine Turbocharger Unit",
        "Expansion Muffler Silencer Box",
        "High-Efficiency Water-Cooled Turbocharger",
        "Cast Iron Segmented Exhaust Manifold Log",
        "Forced Induction Turbocharger System",
        "Cylindrical Cabin Exhaust Silencer Unit",
        "High-Pressure Engine Turbo Unit Assembly",
        "Cast Steel Multi-Port Exhaust Manifold Rig",
        "High-Output Lubricated Turbocharger Setup",
        "Spark-Arrestor Exhaust Silencer Core",
        "Premium Turbocharger Rebuild Internals",
        "Heavy Machinery Low-Noise Exhaust Muffler",
        "Engine Air Intake Turbocharger Array",
        "Structural Exhaust Silencer Muffler Rig",
        "Flexible Stainless Steel Exhaust Pipe Tube",
        "Heavy Duty Turbocharger & Gasket Set",
        "Cast Exhaust Manifold Log Structure",
    ],
    key=len,
    reverse=True,
)
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Gasket" in d or "Clamp" in d or "Bellows" in d or "Fastener" in d or "Pipe" in d:
        return "Exhaust Mounting / Hardware"
    if "Muffler" in d or "Silencer" in d:
        return "Muffler / Silencer"
    if "Manifold" in d:
        return "Exhaust Manifold"
    if "Turbo" in d or "Wastegate" in d or "Actuator" in d:
        return "Turbocharger"
    return d
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A36") or code in section_codes:
                continue
    ...
    s27 = sum(1 for p in products if p.get("section") == "Section 27")
    payload = {
        "status": "sections_2_to_27_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section27": s27,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section27_raw={len(rows)} section27={s27} kept={kept} total={len(products)}")
    for code in ["A01-1", "A35-1", "A36-1", "A36-3A", "A36-9", "A36-14A", "A36-10C", "A36-23", "A36-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
