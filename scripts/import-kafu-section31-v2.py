"""
Import Kafu Section 31 (swing motors, travel motors, final drive).
Upserts A40; keeps prior sections.
"""
...
RAW = ROOT / "data" / "kafu-section31-v2-raw.txt"
...
DESCRIPTIONS = sorted(
    [
        "Hydraulic Upper Structure Swing Motor",
        "Dual-Displacement Final Drive Travel Motor",
        "Travel Motor Dynamic Relief Valve Stem",
        "Planetary Gear Reduction Carrier Block",
        "Mechanical Parking Brake Friction Disc",
        "Rotary Barrel & Piston Shoe Repair Kit",
        "Upper Frame Swing Motor Assembly",
        "Track Assembly Final Drive Travel Motor",
        "Motor Brake Release Valve Cartridge",
        "Upper Structure Swing Motor Assembly",
        "Final Drive Track Travel Motor Unit",
        "Swing Motor Safety Relief Valve Cartridge",
        "Complete Final Drive Motor & Gearbox Unit",
        "Travel Motor Speed Selection Valve",
        "Swing Motor Rotary Group Assembly",
        "Track Travel Motor Assembly Unit",
        "Dual Mechanical Swing Relief Valve Block",
        "Planetary First-Stage Sun Gear Hub",
        "Upper Structure Hydraulic Swing Motor",
        "Track Drive Travel Motor Assembly Block",
        "High-Torque Axial Piston Swing Motor",
        "Travel Motor Anti-Cavitation Valve Stem",
        "Upper Frame Hydraulic Swing Motor",
        "High-Displacement Travel Motor Core",
        "Swing Motor Parking Brake Disc Set",
        "Dual Displacement Track Motor Block Unit",
        "Upper Frame Swing Drive Motor Rig",
        "Final Drive Planetary Reduction Gear Set",
        "Track Final Drive Motor & Reduction Assembly",
        "Swing Drive Overload Brake Valve Unit",
    ],
    key=len,
    reverse=True,
)
...
def parse_oems(raw: str) -> list[str]:
    ...
    raw = re.sub(
        ...
        r"Module Spec|Component Spec|Core Spec|Overhaul Unit|Component Kit|"
...
def normalize_category(desc: str) -> str:
    d = desc
    if "Brake" in d or "Disc" in d:
        return "Parking Brake / Disc"
    if "Planetary" in d or "Carrier" in d or "Sun Gear" in d or "Gear Set" in d or "Gearbox" in d:
        return "Final Drive Gear / Carrier"
    if "Relief" in d or "Valve" in d or "Cartridge" in d:
        return "Motor Relief / Control Valve"
    if "Travel" in d or "Track" in d or "Final Drive" in d:
        return "Travel / Final Drive Motor"
    if "Swing" in d:
        return "Swing Motor"
    if "Rotary" in d or "Repair Kit" in d:
        return "Motor Rotary / Rebuild Kit"
    return d
...
def split_glued(rest: str) -> tuple[str, str, str, str | None]:
    ...
    for m in re.finditer(rf"\b(?:{BRANDS})", rest, re.I):
        ...
        if re.search(r"\d", before) or re.search(
            r"\b(?:VOE|YN|YT|LC|LQ|KHR|AT|RE|ME|S\d|M2X|M5X|KMF)\b", before, re.I
        ):
            bm = m
            break
    if bm is not None:
...
def main() -> None:
    ...
    if OUT_JSON.exists():
        ...
        for p in prev.get("products", []):
            ...
            if code.startswith("A40") or code in section_codes:
                continue
    ...
    s31 = sum(1 for p in products if p.get("section") == "Section 31")
    payload = {
        "status": "sections_2_to_31_v2",
        "supplier": "Kafu",
        "rawRows": len(rows),
        "section31": s31,
        "kept_prior": kept,
    ...
    write_ts(products)
    print(f"section31_raw={len(rows)} section31={s31} kept={kept} total={len(products)}")
    for code in ["A01-1", "A39-1", "A40-1", "A40-3A", "A40-9", "A40-14A", "A40-10C", "A40-23", "A40-24"]:
        p = by_code.get(code)
        if not p:
            print(code, "MISSING")
        else:
